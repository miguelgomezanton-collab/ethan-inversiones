...
async function fetchFearGreed() {
  ...
  // Fallback: calcular Fear & Greed sintético desde VIX
  // VIX < 12 → extrema codicia (85-100)
  // VIX 12-16 → codicia (60-85)
  // VIX 16-20 → neutral (40-60)
  // VIX 20-28 → miedo (20-40)
  // VIX > 28 → extremo miedo (0-20)
  try {
    const vixUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/%5EVIX?interval=1d&range=5d';
    const proxies = [
      u => `https://soft-field-156f.miguel-gomez-anton.workers.dev/?url=${encodeURIComponent(u)}`,
    ...
    for (const fn of proxies) {
      try {
        ...
        let score;
        if (vix < 12)      score = Math.round(85 + (12 - vix) * 1.25);
        else if (vix < 16) score = Math.round(60 + (16 - vix) * 6.25);
        else if (vix < 20) score = Math.round(40 + (20 - vix) * 5);
        else if (vix < 28) score = Math.round(20 + (28 - vix) * 2.5);
        else               score = Math.max(0, Math.round(20 - (vix - 28) * 1.5));
        score = Math.max(0, Math.min(100, score));

        const prevScore = (() => {
          if (prevVix < 12)      return Math.round(85 + (12 - prevVix) * 1.25);
          if (prevVix < 16)      return Math.round(60 + (16 - prevVix) * 6.25);
          if (prevVix < 20)      return Math.round(40 + (20 - prevVix) * 5);
          if (prevVix < 28)      return Math.round(20 + (28 - prevVix) * 2.5);
          return Math.max(0, Math.round(20 - (prevVix - 28) * 1.5));
        })();

        const label = score >= 75 ? 'Extreme Greed' : score >= 55 ? 'Greed' :
                      score >= 45 ? 'Neutral' : score >= 25 ? 'Fear' : 'Extreme Fear';

        return { value: score, label, previousClose: prevScore,
          vix: vix.toFixed(1), source: 'VIX sintético' };
      } catch {}
    }
  } catch {}
