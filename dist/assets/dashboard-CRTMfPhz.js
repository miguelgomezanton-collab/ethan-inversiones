import{U as y}from"./userdata-DRc3raHj.js";import{n as L}from"./index-Bux_OP-y.js";const k=100,z=i=>i!=null&&isFinite(i)?(i<0?"-":"")+"€"+Math.abs(i).toLocaleString("es-ES",{minimumFractionDigits:0,maximumFractionDigits:0}):"—",V=(i,v=2)=>i!=null&&isFinite(i)?(i>=0?"+":"")+i.toFixed(v)+"%":"—",ut=i=>i!=null?i.toFixed(4):"—";async function ft(){try{const i=await fetch("/api/noticias",{signal:AbortSignal.timeout(1e4)});return i.ok?(await i.json()).items||[]:[]}catch{return[]}}function ht(i){if(!i||i.length<2)return"";const v=i.map(n=>n.val),x=200,b=50,r=Math.min(...v),m=Math.max(...v),e=m-r||1,$=v.map((n,M)=>`${(M/(v.length-1)*x).toFixed(1)},${(b-(n-r)/e*b).toFixed(1)}`).join(" "),c=v[v.length-1],l=c>=100?"#40d9c0":"#f47174";return`<svg viewBox="0 0 ${x} ${b}" style="width:100%;height:${b}px;">
